...
| `existing_post_id` | optional | Promote an existing post from **your own Page** instead of creating a new creative. Paste the numeric Post ID (the script combines it with this row's `page_id`), or paste the full `PAGE_ID_POST_ID` form. When set, every other creative column on the row (`image_url`, `video_id`, `primary_text`, `headline`, `description`, `link_url`, `display_link`, `cta`, `browser_addon`, `phone_number`) is ignored — the post already has its content. |
| `partnership_ad_code` | optional | Run a post from a **partner Page** (Branded Content / Partnership Ad) using the ad code they shared with you. Must be the **full `PARTNER_PAGE_ID_POST_ID`** form (with the underscore) — the partner's Page ID is required since it's not your own. Mutually exclusive with `existing_post_id`. Same content-column-ignore rule as above. |
